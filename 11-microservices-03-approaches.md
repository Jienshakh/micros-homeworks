...
### Стек Prometheus + Grafana + cAdvisor
...
```mermaid
graph TD
    A[Микросервисы] --> B[Prometheus exporters]
    C[cAdvisor] --> B
    D[Node Exporter] --> B
    B --> E[Prometheus]
    E --> F[Grafana]
...
| Метрики ресурсов сервисов | cAdvisor (per-container CPU, RAM, HDD, Network) |
| Специфичные метрики сервисов | Custom Prometheus exporters |
| UI для запросов и агрегации | Grafana Explore + PromQL |
| Настройка панелей мониторинга | Grafana dashboards |

### Обоснование выбора

- **Prometheus** - стандарт для мониторинга микросервисов, мощный PromQL
- **cAdvisor** - встроен в Docker, детальные метрики контейнеров
- **Node Exporter** - стандарт для системных метрик
- **Grafana** - лучший UI для визуализации, богатые возможности дашбордов

### Плюсы решения
- Единый стек для инфраструктуры и приложений
- Автоматическое обнаружение сервисов (service discovery)
- Мощный язык запросов PromQL
- Поддержка алертинга в Prometheus/Grafana
